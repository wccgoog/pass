JSONObject parJson = new JSONObject();
                            ...
                            String entryTypeList = sendGet(url,"iw-apikey=123&iw-cmd=getPolicyItemTree");
                            ...
                            for (int i = 0; i < entryTypedata.size(); i++) {
                                if(entryTypedata.getJSONObject(i).get("name").equals(oldappmap.get("policyEntryType"))){
                                    entryTypeCode = entryTypedata.getJSONObject(i).get("id").toString();
                                }
                            }
                            ...
                            parJson.put("frzzhm",application.getApplicantDocumentNumber());//证照号码  //frzzhm
                            parJson.put("dwlxr",application.getApplicantName());//联系人(项目申报单位)  //dwlxr
                            parJson.put("sjhm",application.getPhone());//手机号码(项目申报单位)       //sjhm
                            parJson.put("xmxxdz",oldappmap.get("buildDetailPlace"));//详细地址
                            String backResult =  InterfaceHttpUtils.sendHttpPost(url+"updateRecordHZWZ", parJson.toString());
                            System.out.println("请求body:"+parJson.toString());
                            System.out.println("核准外资修改信息返回结果:"+backResult);