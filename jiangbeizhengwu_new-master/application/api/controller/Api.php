...
//        $this->token   = $this->request->header('AppAuthorization');
        ...
        if (true == $this->needAuth) {
            ...
            $session3rd = isset($this->param['session3rd']) ? $this->param['session3rd'] : '';

            $loginSession = Session::get($session3rd);

            if ((!$session3rd || !$loginSession['user_id'])) {
                return $this->error('登录状态失效,请重新登录');
            }

            //判断是否为维修师傅
            if (!isset($loginSession['workman_id']) || empty($loginSession['workman_id'])) {
                return $this->error('该小程序只能维修师傅使用，请验证手机号');
            }

            $this->user_id = $loginSession['user_id'];

            $this->workman_id = $loginSession['workman_id'];
        }
    }
